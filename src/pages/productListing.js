...
export const ProductListing = () => {
  const {sortBy,dispatch,RatedData,toggleCategory,toggleRating } = useProductContext();
  ...
  return (
    <div className="wrapper">
      <aside className="side-bar">
        ...
        <div className="filter-options">
          <div>
            <div className="options-checkbox">
              <input type="checkbox" onChange={()=>dispatch({type:"TOGGLE_CATEGORY",payload:"RINGS"})}
            ...
            <div className="options-checkbox">
              <input type="checkbox" 
              onChange={()=>dispatch({type:"TOGGLE_CATEGORY",payload:"BRACELETS"})}
              checked={toggleCategory.includes("BRACELETS")}/>
              <label>BRACELETS</label>
            </div>
            <div className="options-checkbox">
              <input type="checkbox" 
              onChange={()=>dispatch({type:"TOGGLE_CATEGORY",payload:"NECKPIECES"})}
              checked={toggleCategory.includes("NECKPIECES")}/>
              <label>NECKPIECES</label>
            </div>
            <div className="options-checkbox">
              <input type="checkbox" 
              onChange={()=>dispatch({type:"TOGGLE_CATEGORY",payload:"EARRINGS"})}
        ...
        <div className="options-radio">
          <input type="radio" onChange={()=>dispatch({type:"TOGGLE_RATING",payload:"4_ABOVE"})} id=""
          checked={toggleRating === "4_ABOVE"} />
          <label>4★ & above</label>
        </div>
        <div className="options-radio">
          <input type="radio" onChange={()=>dispatch({type:"TOGGLE_RATING",payload:"3_ABOVE"})} id=""
          checked={toggleRating === "3_ABOVE"} />
          <label>3★ & above</label>
        </div>
        <div className="options-radio">
          <input type="radio" onChange={()=>dispatch({type:"TOGGLE_RATING",payload:"2_ABOVE"})} id=""
          checked={toggleRating === "2_ABOVE"}/>
          <label>2★ & above</label>
        </div>
        <div className="options-radio">
          <input type="radio" onChange={()=>dispatch({type:"TOGGLE_RATING",payload:"1_ABOVE"})} id=""
          checked={toggleRating === "1_ABOVE"}/>
          <label>1★ & above</label>
        </div>
        <div className="filter-heading">
          <button className="clear-btn" onClick={()=> dispatch({type:"CLEAR_ALL"})}>CLEAR ALL</button>
        </div>
      </aside>
        ...
        {RatedData.map((product) => ( 
          <div className="product-card">
            ...
            <div className="product-card-like">
              <i className="fas fa-heart product-card-like-icon"></i>
            </div>
